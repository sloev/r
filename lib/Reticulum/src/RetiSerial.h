...
class SerialInterface : public Interface {
    ...
    bool esc=false;
public:
    // FIX: Added MTU 500 (Standard RNS Serial MTU)
    SerialInterface(Stream* st) : Interface("Serial", 500), s(st) {}

    void sendRaw(const std::vector<uint8_t>& d) override {
        s->write(FEND); 
        // FIX: Cast 0x00 to uint8_t to resolve ambiguity
    ...
    void loop() {
        while(s->available()) {
            uint8_t b = s->read();
            if(b==FEND) { 
                if(buf.size()>1) receive({buf.begin()+1, buf.end()}); 
                buf.clear(); 
                esc=false; 
            }
            else if(b==FESC) esc=true;
            else { 
                if(esc) { b=(b==TFEND)?FEND:FESC; esc=false; } 
                buf.push_back(b); 
            }
        }
    }
